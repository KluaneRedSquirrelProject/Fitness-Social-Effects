...
fig2a <- ggplot(filter(vis_nonmast_repro$fit), aes(std_soc_repro, visregFit))+
...
fig2b <- ggplot(filter(vis_mast_repro$fit), aes(std_soc_repro, visregFit))+
  geom_line(colour = 'black', 
            size=1)+
...
png("figures/Fig2.png", width = 6000, height = 3000, units = "px", res = 1000)
